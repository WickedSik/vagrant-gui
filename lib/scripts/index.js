...
function tplServer(server) {
    ...
    $('.remove', s).on('click', function(eve) {
        var confirmationText = 'Are you sure you want to remove this server? This will not remove any files!';

        eve.preventDefault();

        if(confirm(confirmationText)) {
            servers = servers.filter(function(item) {
                return item.path !== server.path;
            });
            localStorage.setItem('servers', JSON.stringify(servers));
        }
